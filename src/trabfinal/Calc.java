...
//                                    frame.setVisible(true);
        ...
        if ( GuiTrabFinal.countadou == 0 ){       
        chart.setVisible(false);
        chart.pack();
        RefineryUtilities.centerFrameOnScreen(chart);
        chart.setVisible(true);
        chart.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
        System.err.println("pei_" + GuiTrabFinal.countadou );
        GuiTrabFinal.countadou++;
        System.err.println("peix_" + GuiTrabFinal.countadou );
        }else{
            ArrayList<Double> peixe1 = new ArrayList<>();
            ArrayList<Double> peixe2 = new ArrayList<>();
          for (int i = 0; i < 100; i++){
            peixe1.add((double)i/2);
            peixe2.add((double)i/3);
        }
            chart.updateChart( "Chart", "eth", peixe1, peixe2, cleanDate(sMAETH10date) );
            System.out.println("Peixe podre");
            chart.pack();
            RefineryUtilities.centerFrameOnScreen(chart);
            //chart.setVisible(true);
            chart.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
